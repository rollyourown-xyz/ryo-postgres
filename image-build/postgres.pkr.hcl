...
#------------------------------------------------------------------
# Packer template for building LXD container image for postgres
...
# Specify the postgres version to use in the image build
variable "postgres_version" {
  description = "Mandatory: The postgres version to use in the image build."
  type        = string
}
...
locals {
  ...
  build_playbook_file  = "${abspath(path.root)}/playbooks/provision-postgres.yml"
  build_extra_vars     = "host_id=${var.host_id} module_id=${local.module_id} postgres_version=${var.postgres_version}"
}
